...
namespace MelisPlatformFrameworkSymfony\Service;
...
class MelisPlatformFrameworkSymfonyService
{
        ...
        if (!empty($tableConfig)) {
            ...
            $dtJScript = 'window.' . $fnName . ' = function() {
                ...
                var ' . str_replace("#", "$", $tableId) . ' = $("' . $tableId . '").DataTable({
                    ...
                    columns: ' . $jsonColumns . ',
				    language: '. json_encode($this->getTableTranslations()). ',
                    sDom : \'' . $sDomStructure . '\',
                    bSort: true,
                    searchDelay: 1500,
			        columnDefs: [
     ...
     * @return string
     */
    private function getViewContent($config)
    {
        if($this->container->has('twig')){
            if(!empty($config['view'])) {
                return $this->container->get('twig')->render($config['view'], []);
            }else{
                return 'Cannot render view';
            }
        }else{
            return 'Twig templating is not available';
        }
    }
